...
test('добавление modal-active', () => {
  ...
  expect(modalMessage.modalWrapperEl.classList.contains('modal-active')).toBeTruthy();
});

test('клик на крестик и удаление modal-active', () => {
  modalMessage.showMessage('message');
  modalMessage.closeModal();
  expect(modalMessage.modalWrapperEl.classList.contains('modal-active')).toBeFalsy();
});
